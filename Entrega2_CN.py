...
def primer_inciso(nombres): 
    alumnos_calificados = {alumno:[nota1,nota2] for alumno,nota1,nota2 in zip(nombres,notas_1,notas_2)}
    return alumnos_calificados

alumnos_calificados = primer_inciso(nombres)

def segundo_inciso(alumnos_calificados):
    promedio_alumnos = {}
    for alumno in alumnos_calificados.keys():
        promedio = sum(alumnos_calificados[alumno])/len(alumnos_calificados[alumno])
        promedio_alumnos[alumno] = promedio
    return promedio_alumnos


def tercer_inciso(alumnos_calificados):
    promedio_general = sum(alumnos_calificados.values())/len(alumnos_calificados)
